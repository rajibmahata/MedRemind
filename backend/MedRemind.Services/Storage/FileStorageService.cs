...
public class FileStorageService : IFileStorageService
{
    ...
    public FileStorageService(IOptions<FileStorageConfiguration> config)
    {
        ...
        // Create directory paths: files/prescriptions, files/OCRs, and files/LLMResponses
        ...
        _llmResponsesDirectory = Path.Combine(filesDirectory, "LLMResponses");
        ...
        System.Diagnostics.Debug.WriteLine($"   LLM Responses: {_llmResponsesDirectory}");
    ...
    private void EnsureDirectoriesExist()
    {
        try
        {
            ...
            if (!Directory.Exists(_llmResponsesDirectory))
            {
                Directory.CreateDirectory(_llmResponsesDirectory);
                System.Diagnostics.Debug.WriteLine($"✅ Created LLM responses directory: {_llmResponsesDirectory}");
            }
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Save LLM parser response to file for debugging and audit trail
    /// Format: {Provider}_{prescriptionFileName}.json
    /// Example: OpenAI_user_1_20250129_143022.json
    /// </summary>
    /// <param name="provider">LLM provider name (OpenAI, DeepSeek, Claude)</param>
    /// <param name="jsonResponse">JSON response from the LLM</param>
    /// <param name="prescriptionFileName">Prescription file name to associate with</param>
    /// <returns>Full path where response was saved</returns>
        ...
        string prescriptionFileName)
    {
        if (!_config.EnableFileLogging)
        {
            System.Diagnostics.Debug.WriteLine("📁 File logging is disabled, skipping LLM response save");
            return string.Empty;
        }

        try
        {
            // Remove extension from prescription file name if present
            var baseFileName = Path.GetFileNameWithoutExtension(prescriptionFileName);
            
            // Create file name: Provider_prescriptionFileName.json
            ...
            System.Diagnostics.Debug.WriteLine($"✅ Saved {provider} LLM response:");
            System.Diagnostics.Debug.WriteLine($"   File: {fileName}");
            System.Diagnostics.Debug.WriteLine($"   Size: {jsonResponse.Length} bytes");

            return filePath;
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"❌ Failed to save LLM response: {ex.Message}");
            return string.Empty;
        }
    }

    /// <summary>
    /// Save prescription analysis report generated by validation agent
    /// Format: Analysis_prescriptionFileName.json
    /// Example: Analysis_user_1_20250129_143022.json
    /// </summary>
    public async Task<string> SaveAnalysisReportAsync(
        string reportJson,
        string prescriptionFileName)
    {
        if (!_config.EnableFileLogging)
        {
            System.Diagnostics.Debug.WriteLine("📁 File logging is disabled, skipping analysis report save");
            return string.Empty;
        }
        ...
        try
        {
            ...
            // Create file name: Analysis_prescriptionFileName.json
            var fileName = $"Analysis_{baseFileName}.json";
            var filePath = Path.Combine(_llmResponsesDirectory, fileName);

            // Save analysis report
            await File.WriteAllTextAsync(filePath, reportJson);

            System.Diagnostics.Debug.WriteLine($"✅ Saved analysis report:");
            System.Diagnostics.Debug.WriteLine($"   File: {fileName}");
            System.Diagnostics.Debug.WriteLine($"   Size: {reportJson.Length} bytes");

            return filePath;
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"❌ Failed to save analysis report: {ex.Message}");
            return string.Empty;
        }
    }
}
